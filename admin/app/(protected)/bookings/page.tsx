...
export default function BookingsPage() {
  ...
  const listState = useApiData<BookingRequest[]>(
  ...
  const detailState = useApiData<BookingRequest | null>(
    () =>
      selectedId
        ? api.get<BookingRequest>(`/bookings/${selectedId}`)
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((item) => {
      ...
      const preferred = item.preferredDate
        ? new Date(item.preferredDate)
        : null;
      const diffDays = preferred
        ? Math.ceil(
            (preferred.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
          )
        : null;
      const matchesDate =
        dateFilter === "all" ||
  ...
  async function updateStatus(id: string, status: string) {
    const updated = await api.patch<BookingRequest>(`/bookings/${id}/status`, {
      status,
    });
  ...
  if (listState.error || !listState.data) {
    return (
      <ErrorState
        description={listState.error ?? "Unable to load booking requests"}
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <DetailsDrawer
        ...
        title={
          selected ? getName(selected.customer ?? undefined) : "Booking details"
        }
