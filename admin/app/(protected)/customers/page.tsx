...
export default function CustomersPage() {
  ...
  const listState = useApiData<Customer[]>(
  ...
  const detailState = useApiData<Customer | null>(
    () =>
      selectedId
        ? api.get<Customer>(`/customers/${selectedId}`)
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((item) => {
      const haystack = [getName(item), item.email, item.phone]
        .join(" ")
        .toLowerCase();
  ...
  if (listState.error || !listState.data) {
    return (
      <ErrorState description={listState.error ?? "Unable to load customers"} />
    );
  }
    ...
    <div className="space-y-6">
      ...
      >
        {detailState.isLoading && selectedId ? (
          <LoadingSpinner label="Loading customer history..." />
        ) : null}
        {selected ? (
          <>
              ...
              <div>
                ...
                <ul className="mt-2 space-y-2 text-sm text-slate-600">
                  {(selected.quoteRequests ?? []).map((item) => (
                    <li key={item.id} className="rounded-xl border p-3">
                      <p className="font-medium">
                        {item.service?.name ?? "Quote request"}
                      </p>
                      <p className="mt-1 text-xs text-slate-500">
                        {formatDateTime(item.createdAt)}
                      </p>
              ...
              <div>
                ...
                <ul className="mt-2 space-y-2 text-sm text-slate-600">
                  {(selected.bookingRequests ?? []).map((item) => (
                    <li key={item.id} className="rounded-xl border p-3">
                      <p className="font-medium">
                        {item.service?.name ?? "Booking request"}
                      </p>
                      <p className="mt-1 text-xs text-slate-500">
                        {formatDateTime(item.createdAt)}
                      </p>
